...
namespace axe
{
    ...
    static const char* s_LightingFrag = R"(
        ...
        void main()
        {
            ...
            float roughness = 0.5; // TODO: roughness no G-Buffer
        ...
        const SceneEnvironment* environment)
    {
        ...
        if (ssaoTextureID != 0)
        {
            glBindTextureUnit(3, ssaoTextureID);
            m_Shader->SetInt("u_SSAO", 3);
            m_Shader->SetInt("u_HasSSAO", 1);
        }
        else m_Shader->SetInt("u_HasSSAO", 0);

        // Shadow
        if (shadowMapID != 0)
        {
            glBindTextureUnit(4, shadowMapID);
            m_Shader->SetInt("u_ShadowMap", 4);
        ...
        else m_Shader->SetInt("u_HasLight", 0);
        ...
        {
            environment->Skybox->BindIrradiance(5);
            environment->Skybox->BindPrefiltered(6);
            environment->Skybox->BindBRDFLut(7);
            m_Shader->SetInt("u_IrradianceMap", 5);
            m_Shader->SetInt("u_PrefilteredMap", 6);
